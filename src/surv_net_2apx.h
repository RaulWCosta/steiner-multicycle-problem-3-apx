...
class LPSolver {
 public:
  ...
  void solve(ListGraph::EdgeMap<float>* cap,
             long long int* gomory_hu_total_execution_time) {
    ...
    while (!valid_relaxed_solution) {
      ...
      (*gomory_hu_total_execution_time) +=
          (long long int)std::chrono::duration_cast<std::chrono::milliseconds>(
              end_time - start_time)
              .count();
      ...
      for (int source = 0; source < half_n; source++) {
        ...
        GRBLinExpr expr = 0;
        for (GomoryHu<ListGraph, ListGraph::EdgeMap<float>>::MinCutEdgeIt e(
                 ght, u, v);
             e != INVALID; ++e) {
...
GRBModel* init_gurobi_model(int n, GRBVar* edge_vars, ListGraph& graph,
  ...
  try {
    ...
    for (ListGraph::EdgeIt e(graph); e != INVALID; ++e) {
      ...
      edge_vars[edge_id] = model->addVar(0.0, 1.0, dist, GRB_CONTINUOUS,
                                         "edge_" + itos(edge_id));
    }
...
void round_up_relaxed_solution(int n, ListGraph& graph,
                               ListGraph::EdgeMap<float>& lp_sol,
                               GRBVar* edge_vars, GRBModel& model,
                               int** curr_sol) {
  for (int i = 0; i < n; i++)
    for (int j = 0; j < n; j++) curr_sol[i][j] = 0;
  ...
  for (ListGraph::EdgeIt e(graph); e != INVALID; ++e) {
    ...
    curr_sol[left][right] +=
        (int)(lp_sol[e] + 0.5001);  // account for rounding error
    curr_sol[right][left] = curr_sol[left][right];

    if (curr_sol[left][right])
      model.addConstr(edge_vars[edge_id] == 1.0,
                      "add_edge_(" + itos(edge_id) + ")");
  }
...
void init_cost_map(int n, ListGraph* graph, float** edges_weights,
                   ListGraph::EdgeMap<float>* cost) {
  for (ListGraph::EdgeIt e(*graph); e != INVALID; ++e) {
    int left = graph->id(graph->u(e));
    int right = graph->id(graph->v(e));
...
Result solve(int n, ListGraph* graph, ListGraph::EdgeMap<float>* cost,
             float** edges_weights, GRBVar* edge_vars,
             ListGraph::EdgeMap<float>* lp_sol, int** int_solution) {
  for (int i = 0; i < n; i++)
    for (int j = 0; j < n; j++) int_solution[i][j] = 0;
  ...
  // create model and add "base" restrictions
  ...
  while (!flag_valid_solution) {
    ...
    // add values >= 0.5 on int_solution
    round_up_relaxed_solution(n, *graph, *lp_sol, edge_vars, *model,
                              int_solution);
