...
double** solve(int n, float** edges_weights, FullGraph* graph,
               FullGraph::EdgeMap<float>* cost, GRBEnv* env, GRBVar** edge_vars,
               double** sol) {
  for (int i = 0; i < n; i++)
    for (int j = 0; j < n; j++) sol[i][j] = 0.0f;
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j <= i; j++) {
      edge_vars[i][j] =
          model.addVar(0.0, 2.0, edges_weights[i][j], GRB_CONTINUOUS,
                       "x_" + itos(i) + "_" + itos(j));
