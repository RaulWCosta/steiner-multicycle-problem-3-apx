...
class FeasibleSolCallback : public GRBCallback {
 public:
  ...
  ~FeasibleSolCallback() { delete _cap; }
...
int** solve(int n, float** edges_weights, int** int_sol) {
  ...
  for (int i = 0; i < n; i++)
    for (int j = 0; j < n; j++) int_sol[i][j] = 0;
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j <= i; j++) {
      edge_vars[i][j] = model.addVar(0.0, 2.0, edges_weights[i][j], GRB_INTEGER,
                                     "x_" + itos(i) + "_" + itos(j));
  ...
  for (int i = 0; i < n; i++) edge_vars[i][i].set(GRB_DoubleAttr_UB, 0);
  ...
  for (int i = 0; i < n; i++)
    for (int j = 0; j < n; j++) int_sol[i][j] = (int)(sol[i][j] + 0.001);
