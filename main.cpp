...
Result execute_linear_relaxation(int n, float** edges_weights,
                                 double** double_sol) {
  // allocate memory
  ...
  for (int i = 0; i < n; i++) edge_vars[i] = new GRBVar[n];

  auto start_time = std::chrono::high_resolution_clock::now();
  double_sol = LinearSMCP::solve(n, edges_weights, graph, cost, env, edge_vars,
                                 double_sol);
  auto end_time = std::chrono::high_resolution_clock::now();
  auto relaxed_time = std::chrono::duration_cast<std::chrono::milliseconds>(
                          end_time - start_time)
                          .count();

  double relaxed = get_solution_value(n, double_sol, edges_weights);

  for (int i = 0; i < n; i++) delete[] edge_vars[i];
  delete[] edge_vars;
...
std::tuple<Result, Result, Result, Result> execute_approximation(
    int n, float** edges_weights, int** int_sol) {
  // allocate memory
  ...
  struct Result result_gomory_hu = SurvivableNetwork::solve(
      n, graph, cost, edges_weights, edge_vars, aux_lp_sol, int_sol);
  auto end_time = std::chrono::high_resolution_clock::now();
  ...
  return std::make_tuple(result_sn, result_gomory_hu, result_perf_match,
                         result_shortcutting);
}
...
int main(int argc, char* argv[]) {
  ...
  for (string& file : *files) {
    ...
    Result linear_relaxation_result =
        execute_linear_relaxation(n, edges_weights, double_sol);

    Result sn_result, result_gomory_hu, perf_match_result, short_cutting_result;
    std::tie(sn_result, result_gomory_hu, perf_match_result,
             short_cutting_result) =
        execute_approximation(n, edges_weights, int_sol);

    struct ExecutionTracker tracker = {struct Instance{file, n},
  ...
  for (int i = 0; i < MAX_INSTANCE_SIZE; i++) delete[] double_sol[i];
  delete[] double_sol;

  for (int i = 0; i < MAX_INSTANCE_SIZE; i++) delete[] edges_weights[i];
  delete[] edges_weights;

  for (int i = 0; i < MAX_INSTANCE_SIZE; i++) delete[] int_sol[i];
  delete[] int_sol;
